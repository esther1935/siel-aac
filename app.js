...
async function registerServiceWorker() {
  ...
  try {
    await navigator.serviceWorker.register("./sw.js?v=sielFinalKbTeacherFix20260628");
    updateSyncStatus();
  } catch (e) {
...
async function initFirebase() {
  try {
    const configModule = await import("./firebase-config.js?v=sielFinalKbTeacherFix20260628");
...
/* FINAL KB + TEACHER PERMISSION FIX 20260628
   - 일반 AAC 카드에는 KB 용량이 보이지 않게 함
   - 선생님/기관별 업로드는 승인 없이 바로 저장
   - 선생님 화면에는 삭제 버튼 없음
   - 삭제는 엄마 전체 관리자에서만 관리 */

function sielFinalTeacherFindCategory(name) {
  const target = String(name || "").trim();
  return data.categories.find(cat => String(cat.name || "").trim() === target);
}

function sielFinalTeacherEnsureCategory(name) {
  let cat = sielFinalTeacherFindCategory(name);
  if (!cat) {
...
function sielFinalTeacherFileToDataUrl(file) {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = () => resolve(reader.result);
    reader.onerror = reject;
...
async function sielFinalTeacherOptimizeFile(file) {
  if (!file) return file;
  try {
    if (typeof compressImageFile === "function") return await compressImageFile(file);
  } catch (e) {
...
function sielFinalTeacherFormatSize(bytes) {
  const n = Number(bytes || 0);
  if (!n || Number.isNaN(n)) return "";
  if (n < 1024) return `${n} B`;
  if (n < 1024 * 1024) return `${Math.round(n / 1024)} KB`;
...
async function sielFinalTeacherSaveCard(role, text, file, originalFile) {
  const cat = sielFinalTeacherEnsureCategory(role.category);
  const dataUrl = await sielFinalTeacherFileToDataUrl(file);
  const card = {
    id: crypto.randomUUID(),
    text,
    speak: text,
    ...
    originalSize: originalFile && originalFile.size ? originalFile.size : 0,
    imageType: file.type || "",
    createdBy: role.label || role.id || "",
    updatedAt: Date.now()
  };
  cat.cards.push(card);
  if (typeof saveData === "function") saveData();
  else if (typeof saveLocalOnly === "function") saveLocalOnly();
  if (typeof render === "function") render();
  return card;
}

function sielShowLimitedUploader(role) {
  const el = (typeof sielRoleOverlay === "function") ? sielRoleOverlay() : document.getElementById("sielRoleOverlay");
  if (!el) return;
  ...
  el.innerHTML = `
    <div class="sielRoleCard sielRoleWide">
      ...
      <button class="sielRoleBack" type="button">‹ 역할 선택</button>
      <h2>${sielRoleEscape(role.label)} 업로드</h2>
      <p class="sielRoleSub"><b>${sielRoleEscape(role.category)}</b> 카테고리에 승인 없이 바로 저장됩니다.</p>
      ...
      <button class="sielRolePrimary" type="button">바로 저장</button>
      <p class="sielRoleSub sielRoleStatus"></p>

      <div class="sielTeacherRecentBox">
        <h3>${sielRoleEscape(role.category)} 최근 그림</h3>
        ${recent.length ? recent.map(card => `
          <div class="sielTeacherRecentItem">
            <img src="${(typeof imageDisplayUrl === "function") ? imageDisplayUrl(card) : card.image}" alt="" />
            <span>${sielRoleEscape(card.text || "")}</span>
            <em>${sielFinalTeacherFormatSize(card.compressedSize || card.imageSize || 0)}</em>
          </div>
  ...
  const close = el.querySelector(".sielRoleClose");
  if (close) close.onclick = (typeof sielCloseRoleOverlay === "function") ? sielCloseRoleOverlay : (() => el.remove());

  const back = el.querySelector(".sielRoleBack");
  if (back) back.onclick = (typeof sielShowRoleLogin === "function") ? sielShowRoleLogin : (() => el.remove());
  ...
  el.querySelector(".sielRolePrimary").onclick = async () => {
    ...
    try {
      const file = await sielFinalTeacherOptimizeFile(originalFile);
      await sielFinalTeacherSaveCard(role, text, file, originalFile);
      status.textContent = `바로 저장되었습니다. ${sielFinalTeacherFormatSize(file.size)}`;
      fileInput.value = "";
      nameInput.value = "";
      setTimeout(() => sielShowLimitedUploader(role), 450);
    } catch (e) {
      console.error(e);
      status.textContent = "저장 중 오류가 났습니다.";
    }
...
/* 메인 AAC 화면의 카드 용량 텍스트 제거 */
function sielHideKbOnMainCardsOnly() {
  try {
    const adminOpen = document.querySelector("#adminDialog[open], .sielRoleOverlay, #sielRoleOverlay");
    if (adminOpen) return;

    const candidates = Array.from(document.querySelectorAll("span, em, small, div"));
    candidates.forEach(el => {
      const text = (el.textContent || "").trim();
      if (/^\d+(\.\d+)?\s*(KB|MB|B)$/i.test(text)) {
        const inAdmin = el.closest("#adminDialog, #sielRoleOverlay, .sielTeacherRecentBox, .deleteList, .adminPanel, .modal");
        if (!inAdmin) el.style.display = "none";
      }
    });
  } catch (e) {}
}

setInterval(sielHideKbOnMainCardsOnly, 1000);
document.addEventListener("DOMContentLoaded", () => setTimeout(sielHideKbOnMainCardsOnly, 300));
